...
import InitSeriesInput from './InitSeriesInput';
...
class Incomplete extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.state = {
            lcdSet: [], // Begin with initializing an empty array
        };
    ...
    getLines() {
        ...
        for (let i = 0; i <= maxIndex; i += 1) {
            ...
            set.push({
                id: i+1,
                LCD: thisLCD,
                JDN: getJDNFromLCD(thisLCD, this.props.correlation),
                LongCount: buildLCString(lc),
                CR: crString(cr, this.props.names),
                Lord: getGodNum(lc.winals, lc.kins),
                Gregorian: getJDNDateString(jdnToDate(getJDNFromLCD(thisLCD, this.props.correlation), 'g'), 'g', false),
                Julian:    getJDNDateString(jdnToDate(getJDNFromLCD(thisLCD, this.props.correlation), 'j'), 'j', false),
            });
        }
                    ...
                    <Grid item xs={12}>
                        <Paper sx={ paperStyle }>
                            <Grid container>
                                ...
                                <Grid item xs={12} md={10}>{this.inputContent()}</Grid>
                            </Grid>
                        </Paper>
                    </Grid>
                    ...
                    <Grid item xs={12}>
                        <Paper sx={ paperStyle }>
                           <Grid container>
                                ...
                                <Grid item xs={12}><b>{this.handleHeader()}</b></Grid>
                                <Grid item xs={12}>{'Using correlation factor: ' + this.props.correlation }</Grid>
